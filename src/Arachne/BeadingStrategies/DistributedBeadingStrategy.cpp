...
coord_t DistributedBeadingStrategy::optimalThickness(coord_t bead_count) const
{
    return bead_count * optimal_width;
}

coord_t DistributedBeadingStrategy::transitionThickness(coord_t lower_bead_count) const
{
    return lower_bead_count * optimal_width + optimal_width / 2; // TODO: doesnt take min and max width into account
}

coord_t DistributedBeadingStrategy::optimalBeadCount(coord_t thickness) const
{
